...
from ecommerce3.models import Cohort as Cor
from ecommerce3.models import UserProfile
from django.contrib.auth.models import User
from django.contrib.auth.forms import UserCreationForm

class RegistrationForm(UserCreationForm):
    email = forms.EmailField(label='E-mail', required=True)
    cohort = forms.ModelChoiceField(queryset=Cor.objects.all())
    ...
    def save(self, commit=True):
        ...
        if commit:
            user.save()
            user_profile = UserProfile.objects.create(user=user, cohort=Cor.objects.get(name=cohort))
            user_profile.save()
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['cohort'].widget.attrs['class'] = 'form-control'


class CohortRegistrationForm(forms.Form):
    ...
    def save(self, commit=True):
        cohort = Cor()
        cohort.social_name = self.cleaned_data['social_name']
        cohort.name = self.cleaned_data['name']
        cohort.email = self.cleaned_data['email']
        cohort.city = self.cleaned_data['city']
        cohort.found_date = self.cleaned_data['found_date']
